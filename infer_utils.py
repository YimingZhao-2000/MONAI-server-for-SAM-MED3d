...
def sam_model_infer_with_user_prompt(model,
                                    roi_image,
                                    roi_gt=None,
                                    prompt_generator=random_sample_next_click,
                                    prev_low_res_mask=None,
                                    ...
                                    user_labels=None):
    '''
    Inference for SAM-Med3D with user prompt support
    '''
    model.eval()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = model.to(device)
    ...
    with torch.no_grad():
        ...
        for i in range(num_clicks):
            if roi_gt is not None:
                # training or validation mode with gt
                new_points_co, new_points_la = prompt_generator(
            ...
            elif user_points is not None and user_labels is not None:
                # 推理模式下，用户从 slicer 传入的真实 clicks
                if i == 0:
                    points_coords = user_points
                    points_labels = user_labels
                # 不再采样新的 prompt，直接用用户输入结束
                break
                
            else:
                # fallback：无用户输入/gt，使用中心点
                if points_coords.shape[1] == 0:
                    ...
                    print("Warning: No GT or user click for next prompt; skipping further clicks.")
                    break
